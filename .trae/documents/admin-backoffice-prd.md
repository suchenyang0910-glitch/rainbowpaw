...
面向运营/财务/风控等内部人员，统一对接 identity/wallet/bridge/order/report/risk 六类后端服务。
...
4. **业务控制台**：按服务分区（identity/wallet/bridge/order/report/risk）进行查询与操作。
...
| 业务控制台 | Order 分区 | 查询订单列表与订单详情（来自 order-service）；在订单详情页联动展示“可用于报表的关键字段”（如 orderId、业务时间、金额/币种、状态） |
| 业务控制台 | Report 分区 | 选择条件生成报表（由 report-service 产出）；展示报表任务列表（生成中/成功/失败）；下载/导出；失败重试；对齐展示与订单相关的筛选字段（如 orderId、时间范围） |
...
- 日常操作流程：你从菜单进入业务分区页面 → 进行查询（例如先查订单，再按订单号/时间范围生成报表）→ 若点击写操作按钮（如调账/冻结/审批/重试/失败重试）则先做二次确认 → 调用后端聚合接口转发到目标服务 → 回显结果并写入审计日志。
